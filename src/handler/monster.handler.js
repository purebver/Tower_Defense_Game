...
export const monsterSpawn = (accountId, data) => {
  ...
  // 몬스터 공격력 검증
  if (monster.monsterAttack !== dbMonster.monsterAttack) {
    return { status: 'fail', message: 'monsterAttack Is Strange' };
  }
  // 몬스터 골드 검증
  if (monster.monsterGold !== dbMonster.monsterGold) {
    return { status: 'fail', message: 'monsterGold Is Strange' };
  }
  // 몬스터 스코어 검증
